...
def upload():
    if request.method == 'POST':
        ...
        #result = result+d.get(result,"unknown prediction result")

        result = result.lower()


        d = {
        "brownspot": " → You are Normal. Stay healthy!",
        "normal": " → You are Normal. Stay healthy!",
        "severe": " → Signs of a serious neurodegenerative condition. Medical help is advised.",
        "stage1": " → Early stage of a neuro disorder. Consult a neurologist.",
        "stage2": " → Signs of advanced neuro disorder. Further tests are recommended."}
        result = result + d.get(result, " → Unknown prediction result")
        ...
        return result
        #return result3
    return "Please use a POST request to upload a file."
